QUESTION:   https://hdlbits.01xz.net/mw/images/a/ae/Ece241_2013_q4.png
...
module top_module (
    ...
    //Edge trifggered block (DFF) for state flip-flop. Synchronous Reset
	always @(posedge clk) begin
		if(reset) state <= A;
		else state <= next_state;
	end
    //Combinatonal always block for state transiion logic. For given that current state abd inputs
    //Combinational always block using :- Always assigments.
	always @(*) begin
		case(state)
